...
UINT DX11ScanCS::ScanCS( ID3D11DeviceContext* pd3dImmediateContext, UINT numElements,
	ID3D11ShaderResourceView* input_SRV, ID3D11UnorderedAccessView* input_UAV,
	ID3D11ShaderResourceView* output_SRV, ID3D11UnorderedAccessView* output_UAV,
	ID3D11Buffer* output_Buf)
{
	...
	if (output_Buf) 
	{	
		// if output buffer is set return value
		...
		pd3dImmediateContext->CopySubresourceRegion(m_PrefixSumLastElemStagging, 0, 0, 0, 0, output_Buf, 0, &sourceRegion);
