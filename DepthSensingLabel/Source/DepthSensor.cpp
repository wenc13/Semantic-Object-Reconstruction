...
DepthSensor::DepthSensor()
{
	...
	for(int i=0;i<LABEL_TEXTURE_NUM; i++) m_label[i] = NULL;

	m_preDepthD16 = NULL;
	m_preColorRGBX = NULL;

	for (int i = 0;i < LABEL_TEXTURE_NUM; i++) m_preLabel[i] = NULL;
}
...
void DepthSensor::init(unsigned int depthWidth, unsigned int depthHeight, unsigned int colorWidth, unsigned int colorHeight)
{
	...
	{
		SAFE_DELETE_ARRAY(m_label[i]);
		m_label[i] = new BYTE[m_colorWidth*m_colorHeight*cBytesPerPixel];				//Use R32G32B32A32 format render the label percent
		memset(m_label[i], 0, sizeof(BYTE)*m_colorWidth*m_colorHeight*cBytesPerPixel);
	}

	SAFE_DELETE_ARRAY(m_preDepthD16);
	m_preDepthD16 = new USHORT[m_depthWidth*m_depthHeight];
	memset(m_preDepthD16, 0, sizeof(USHORT)*m_depthWidth*m_depthHeight);
	
	SAFE_DELETE_ARRAY(m_preColorRGBX);
	m_preColorRGBX = new BYTE[m_colorWidth*m_colorHeight*cBytesPerPixel];
	memset(m_preColorRGBX, 0, sizeof(BYTE)*m_colorWidth*m_colorHeight*cBytesPerPixel);

	for (int i = 0; i < LABEL_TEXTURE_NUM; i++)
	{
		SAFE_DELETE_ARRAY(m_preLabel[i]);
		m_preLabel[i] = new BYTE[m_colorWidth*m_colorHeight*cBytesPerPixel];				//Use R32G32B32A32 format render the label percent
		memset(m_preLabel[i], 0, sizeof(BYTE)*m_colorWidth*m_colorHeight*cBytesPerPixel);
	}
}
...
DepthSensor::~DepthSensor()
{
	...
	SAFE_DELETE_ARRAY(m_depthD16);
	SAFE_DELETE_ARRAY(m_colorRGBX);
	for(int i=0;i<LABEL_TEXTURE_NUM; i++) SAFE_DELETE_ARRAY(m_label[i]);

	SAFE_DELETE_ARRAY(m_preDepthD16);
	SAFE_DELETE_ARRAY(m_preColorRGBX);
	for (int i = 0;i < LABEL_TEXTURE_NUM; i++) SAFE_DELETE_ARRAY(m_preLabel[i]);
