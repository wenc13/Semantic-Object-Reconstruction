...
DepthSensor* GlobalAppState::getDepthSensor()
{
	...
	else if(GlobalAppState::getInstance().s_sensorIdx == 8){   //new label image reader
		printf("LABEL MODEL.Noparts: %d\n", GlobalAppState::getInstance().s_labelNum);
		static LabelReaderSensor s_labelReader;
		s_labelReader.setBaseFilePath(GlobalAppState::getInstance().s_ImageReaderSensorSourcePath);
		s_labelReader.setNumFrames(GlobalAppState::getInstance().s_ImageReaderSensorNumFrames);
		return &s_labelReader;
	}
	else {
