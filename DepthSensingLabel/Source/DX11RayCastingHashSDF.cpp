...
ID3D11Texture2D* DX11RayCastingHashSDF::s_pLabels[LABEL_TEXTURE_NUM] = { NULL, NULL, NULL };
ID3D11ShaderResourceView* DX11RayCastingHashSDF::s_pLabelsSRV[LABEL_TEXTURE_NUM] = { NULL, NULL, NULL };
ID3D11UnorderedAccessView* DX11RayCastingHashSDF::s_pLabelsUAV[LABEL_TEXTURE_NUM] = { NULL, NULL, NULL };
...
ID3D11Texture2D* DX11RayCastingHashSDF::s_pLabelsStereo[LABEL_TEXTURE_NUM] = { NULL, NULL, NULL };
ID3D11ShaderResourceView* DX11RayCastingHashSDF::s_pLabelsStereoSRV[LABEL_TEXTURE_NUM] = { NULL, NULL, NULL };
ID3D11UnorderedAccessView* DX11RayCastingHashSDF::s_pLabelsStereoUAV[LABEL_TEXTURE_NUM] = { NULL, NULL, NULL };
...
// test
ID3D11Texture2D* DX11RayCastingHashSDF::s_pTestSplattingMin = NULL;
ID3D11ShaderResourceView* DX11RayCastingHashSDF::s_pTestSplattingMinSRV = NULL;
ID3D11UnorderedAccessView* DX11RayCastingHashSDF::s_pTestSplattingMinUAV = NULL;

ID3D11Texture2D* DX11RayCastingHashSDF::s_pTestSplattingMax = NULL;
ID3D11ShaderResourceView* DX11RayCastingHashSDF::s_pTestSplattingMaxSRV = NULL;
ID3D11UnorderedAccessView* DX11RayCastingHashSDF::s_pTestSplattingMaxUAV = NULL;
// test
...
HRESULT DX11RayCastingHashSDF::Render(ID3D11DeviceContext* context, ID3D11ShaderResourceView* hash, ID3D11ShaderResourceView* hashCompact, ID3D11ShaderResourceView* sdfBlocksSDF, ID3D11ShaderResourceView* sdfBlocksRGBW, ID3D11ShaderResourceView** sdfBlocksLabel, ID3D11ShaderResourceView* sdfBlocksLabelFrequency, unsigned int hashNumValidBuckets, unsigned int renderTargetWidth, unsigned int renderTargetHeight, const mat4f* lastRigidTransform, ID3D11Buffer* CBsceneRepSDF)
{
	return RenderToTexture(context, hash, hashCompact, sdfBlocksSDF, sdfBlocksRGBW, sdfBlocksLabel, sdfBlocksLabelFrequency, hashNumValidBuckets, renderTargetWidth, renderTargetHeight, lastRigidTransform, CBsceneRepSDF,
						   ...
						   m_pSSAOMapSRV, m_pSSAOMapUAV, 
		                   m_pSSAOMapFilteredUAV);
}
...
HRESULT DX11RayCastingHashSDF::RenderStereo(ID3D11DeviceContext* context, ID3D11ShaderResourceView* hash, ID3D11ShaderResourceView* hashCompact, ID3D11ShaderResourceView* sdfBlocksSDF, ID3D11ShaderResourceView* sdfBlocksRGBW, ID3D11ShaderResourceView** sdfBlocksLabel, ID3D11ShaderResourceView* sdfBlocksLabelFrequency, unsigned int hashNumValidBuckets, unsigned int renderTargetWidth, unsigned int renderTargetHeight, const mat4f* lastRigidTransform, ID3D11Buffer* CBsceneRepSDF)
{
	...
	if (GlobalAppState::getInstance().s_stereoEnabled)
	{
...

		{
			...
			V_RETURN(RenderToTexture(context, hash, hashCompact, sdfBlocksSDF, sdfBlocksRGBW, sdfBlocksLabel, sdfBlocksLabelFrequency, hashNumValidBuckets, renderTargetWidth, renderTargetHeight, &lastRigidTransformNew, CBsceneRepSDF,
				...
				s_pColorsStereoUAV,s_pLabelStereoUAV,
				s_pNormalsStereoSRV, s_pNormalsStereoUAV,
				m_pSSAOMapStereoSRV, m_pSSAOMapStereoUAV, m_pSSAOMapFilteredStereoUAV));

			DX11PhongLighting::renderStereo(context, s_pPositionsStereoSRV, s_pNormalsStereoSRV, s_pColorsStereoSRV, s_pLabelStereoSRV, m_pSSAOMapStereoSRV, useColor, false);
...
HRESULT DX11RayCastingHashSDF::RenderToTexture(ID3D11DeviceContext* context, ID3D11ShaderResourceView* hash, ID3D11ShaderResourceView* hashCompact, ID3D11ShaderResourceView* sdfBlocksSDF, ID3D11ShaderResourceView* sdfBlocksRGBW, ID3D11ShaderResourceView** sdfBlocksLabel, ID3D11ShaderResourceView* sdfBlocksLabelFrequency, unsigned int hashNumValidBuckets, unsigned int renderTargetWidth, unsigned int renderTargetHeight, const mat4f* lastRigidTransform, ID3D11Buffer* CBsceneRepSDF,
											   ...
											   ID3D11UnorderedAccessView** pLabelUAV , 
											   ID3D11ShaderResourceView*   pNormalsSRV,                  ID3D11UnorderedAccessView* pNormalsUAV, 
											   ID3D11ShaderResourceView*   pSSAOMapSRV,                  ID3D11UnorderedAccessView* pSSAOMapUAV, 
											   ID3D11UnorderedAccessView*  pSSAOMapFilteredUAV)
{	
	HRESULT hr = S_OK;
	...
	if (false)
	{
		...
		DX11ImageHelper::applySaveAsTexture(context, pDepthStencilSplattingMinSRV, s_pTestSplattingMinUAV, GlobalAppState::getInstance().s_windowWidth, GlobalAppState::getInstance().s_windowHeight);
		DX11ImageHelper::applySaveAsTexture(context, pDepthStencilSplattingMaxSRV, s_pTestSplattingMaxUAV, GlobalAppState::getInstance().s_windowWidth, GlobalAppState::getInstance().s_windowHeight);
		...
		wcscat(fileName, L"Max.jpg");
		D3DX11SaveTextureToFile(context, s_pTestSplattingMax, D3DX11_IFF_JPG, fileName);
	...
	{
		context->CSSetShaderResources(7+labelno, 1, &sdfBlocksLabel[labelno]);
	}
	context->CSSetShaderResources(10, 1, &sdfBlocksLabelFrequency);
	...
	for (int labelno = 0; labelno < LABEL_TEXTURE_NUM; labelno++)
	{
		context->CSSetUnorderedAccessViews(3+labelno, 1, &pLabelUAV[labelno], 0);
	}
	...
	for (int labelno = 0; labelno < LABEL_TEXTURE_NUM; labelno++)
	{
		context->CSSetShaderResources(7+labelno, 1, nullSRV);
	}
	...
	for (int labelno = 0; labelno < LABEL_TEXTURE_NUM; labelno++)
	{
		context->CSSetUnorderedAccessViews(3+labelno, 1, nullUAV, 0);
	}
...
HRESULT DX11RayCastingHashSDF::initialize(ID3D11Device* pd3dDevice)
{
	...
	for (int labelno = 0; labelno < LABEL_TEXTURE_NUM; labelno++)
	{
		V_RETURN(pd3dDevice->CreateTexture2D(&descTex, NULL, &s_pLabels[labelno]));
		V_RETURN(pd3dDevice->CreateShaderResourceView(s_pLabels[labelno], NULL, &s_pLabelsSRV[labelno]));
		V_RETURN(pd3dDevice->CreateUnorderedAccessView(s_pLabels[labelno], NULL, &s_pLabelsUAV[labelno]));
	}
	...
	if (GlobalAppState::getInstance().s_stereoEnabled)
	{
		...
		for (int labelno = 0; labelno < LABEL_TEXTURE_NUM; labelno++)
		{
			V_RETURN(pd3dDevice->CreateTexture2D(&descTex, NULL, &s_pLabelsStereo[labelno]));
			V_RETURN(pd3dDevice->CreateShaderResourceView(s_pLabelsStereo[labelno], NULL, &s_pLabelsStereoSRV[labelno]));
			V_RETURN(pd3dDevice->CreateUnorderedAccessView(s_pLabelsStereo[labelno], NULL, &s_pLabelsStereoUAV[labelno]));
		}
	...
	// test
	D3D11_TEXTURE2D_DESC testTexDesc = { 0 };
	testTexDesc.Width = GlobalAppState::getInstance().s_windowWidth;
	testTexDesc.Height = GlobalAppState::getInstance().s_windowHeight;
	...
	testTexDesc.Usage = D3D11_USAGE_DEFAULT;
	testTexDesc.BindFlags = D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_UNORDERED_ACCESS;
	testTexDesc.CPUAccessFlags = 0;
	testTexDesc.MiscFlags = 0;

	V_RETURN(pd3dDevice->CreateTexture2D(&testTexDesc, NULL, &s_pTestSplattingMin));
	V_RETURN(pd3dDevice->CreateShaderResourceView(s_pTestSplattingMin, NULL, &s_pTestSplattingMinSRV));
	V_RETURN(pd3dDevice->CreateUnorderedAccessView(s_pTestSplattingMin, NULL, &s_pTestSplattingMinUAV));

	V_RETURN(pd3dDevice->CreateTexture2D(&testTexDesc, NULL, &s_pTestSplattingMax));
	V_RETURN(pd3dDevice->CreateShaderResourceView(s_pTestSplattingMax, NULL, &s_pTestSplattingMaxSRV));
	V_RETURN(pd3dDevice->CreateUnorderedAccessView(s_pTestSplattingMax, NULL, &s_pTestSplattingMaxUAV));
	// test
...
void DX11RayCastingHashSDF::destroy()
{
	...
	for (int labelno = 0; labelno < LABEL_TEXTURE_NUM; labelno++)
	{
		SAFE_RELEASE(s_pLabels[labelno]);
		SAFE_RELEASE(s_pLabelsSRV[labelno]);
		SAFE_RELEASE(s_pLabelsUAV[labelno]);

		// Stereo	
		SAFE_RELEASE(s_pLabelsStereo[labelno]);
		SAFE_RELEASE(s_pLabelsStereoSRV[labelno]);
		SAFE_RELEASE(s_pLabelsStereoUAV[labelno]);
	}
	...
	// test
	SAFE_RELEASE(s_pTestSplattingMin);
	SAFE_RELEASE(s_pTestSplattingMinSRV);
	SAFE_RELEASE(s_pTestSplattingMinUAV);

	SAFE_RELEASE(s_pTestSplattingMax);
	SAFE_RELEASE(s_pTestSplattingMaxSRV);
	SAFE_RELEASE(s_pTestSplattingMaxUAV);
	// test
}