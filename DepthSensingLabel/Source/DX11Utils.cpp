#include "d3dcompiler.h"	//hack... (force to use the new hlsl compiler)
...
{
	std::wstring potentialShaderDirectory(L"Shaders\\");
	HRESULT hr = S_OK;
	...
	size_t oldLen = compiledFilename.length();
	...
	if (hFindShader == INVALID_HANDLE_VALUE) {
		std::wstring testName(potentialShaderDirectory);
		testName.append(szFileName);
		hFindShader = FindFirstFile(testName.c_str(), &findData_shader);
	}
	...
	if (!s_bUsePreCompiledShaders || hFindCompiled == INVALID_HANDLE_VALUE || CompareFileTime(&findData_shader.ftLastWriteTime, &findData_compiled.ftLastWriteTime) > 0) {
		...
		if (FAILED(hr)) {
			std::wstring testName(potentialShaderDirectory);
			testName.append(szFileName);
			hr = DXUTFindDXSDKMediaFileCch( str, MAX_PATH, testName.c_str() );
		}
		V_RETURN( hr );
		...
		std::cout << "Compiling Shader " << std::string(fileName.begin(), fileName.end()) << " ( " << entryPoint << " ) " << " ... ";
...
void* CreateAndCopyToDebugBuf( ID3D11Device* pDevice, ID3D11DeviceContext* pd3dImmediateContext, ID3D11Buffer* pBuffer, bool returnCPUMemory )
{
	...
	desc.BindFlags = 0;
	desc.MiscFlags = 0;
	HRESULT hr = pDevice->CreateBuffer(&desc, NULL, &debugbuf);
	if(FAILED(hr)) std::cout<<"FAILED"<< std::endl;

	pd3dImmediateContext->CopyResource( debugbuf, pBuffer );

	INT *cpuMemory = new INT[desc.ByteWidth/sizeof(UINT)];
	D3D11_MAPPED_SUBRESOURCE mappedResource;
	hr = pd3dImmediateContext->Map(debugbuf, D3D11CalcSubresource(0,0,0), D3D11_MAP_READ, 0, &mappedResource);	
	if(FAILED(hr)) std::cout<<"FAILED"<< std::endl;
...
void* CreateAndCopyToDebugTexture2D( ID3D11Device* pDevice, ID3D11DeviceContext* pd3dImmediateContext, ID3D11Texture2D* pBufferTex, bool returnCPUMemory )
{
	...
	for(unsigned int i = 0; i < desc.Height * desc.Width; i++)
	{
		if(cpuMemory[i] != -std::numeric_limits<float>::infinity())	{
			numValid++;
		}
	}
