...
DX11SceneRepHashSDF::DX11SceneRepHashSDF()
{
	...
	// test
	m_HashIntegrateDecisionCPU = NULL;
	// test
	...
	{
		m_SDFBlocksLabel[labelno] = NULL;
		m_SDFBlocksLabelSRV[labelno] = NULL;
		m_SDFBlocksLabelUAV[labelno] = NULL;
	}

	m_SDFBlocksLabelFrequency = NULL;
	m_SDFBlocksLabelFrequencySRV = NULL;
	m_SDFBlocksLabelFrequencyUAV = NULL;
	...
	// test
	m_TestIntegrateColorCPU = NULL;
	m_TestIntegrateLabelCPU = NULL;
	m_TestIntegrateLabelFrequencyCPU = NULL;
	// test
}
...
void DX11SceneRepHashSDF::Destroy()
{
	...
	for (int labelno = 0; labelno < LABEL_TEXTURE_NUM; labelno++)
	{
		SAFE_RELEASE(m_SDFBlocksLabel[labelno]);
		SAFE_RELEASE(m_SDFBlocksLabelSRV[labelno]);
		SAFE_RELEASE(m_SDFBlocksLabelUAV[labelno]);
	}

	SAFE_RELEASE(m_SDFBlocksLabelFrequency);
	SAFE_RELEASE(m_SDFBlocksLabelFrequencySRV);
	SAFE_RELEASE(m_SDFBlocksLabelFrequencyUAV);
	...
	// test
	SAFE_RELEASE(m_HashIntegrateDecisionCPU);
	// test
	...
	// test
	SAFE_RELEASE(m_TestIntegrateColorCPU);
	SAFE_RELEASE(m_TestIntegrateLabelCPU);
	SAFE_RELEASE(m_TestIntegrateLabelFrequencyCPU);
	// test
}
...
void DX11SceneRepHashSDF::Integrate(ID3D11DeviceContext* context, ID3D11ShaderResourceView* inputDepth, ID3D11ShaderResourceView* inputColor, ID3D11ShaderResourceView** inputLabel, ID3D11ShaderResourceView* bitMask, const mat4f* rigidTransform)
{
	...
	Alloc(context, inputDepth, inputColor, inputLabel, bitMask);
	...
	IntegrateDepthMap(context, inputDepth, inputColor, inputLabel);
	...
	//{
	//	if (GetNumIntegratedImages() > 1 && GetNumIntegratedImages() % 15 == 0) 
	//	{	
	//		//reduce one weight every 15 frames
...
void DX11SceneRepHashSDF::RemoveAndIntegrateToOther( ID3D11DeviceContext* context, DX11SceneRepHashSDF* other, const mat4f* lastRigid, bool moveOutsideFrustum )
{
	...
	if (moveOutsideFrustum)
		context->CSSetShader(s_SDFVoxelHashRemoveAndIntegrateOutFrustum, 0, 0);
	else
		context->CSSetShader(s_SDFVoxelHashRemoveAndIntegrateInFrustum, 0, 0);
...
void DX11SceneRepHashSDF::Reset( ID3D11DeviceContext* context )
{
	...
	if (!m_JustHashAndNoSDFBlocks) 
	{
		...
		context->CSSetUnorderedAccessViews(2, 1, &m_SDFBlocksLabelUAV[0], NULL);
		context->CSSetUnorderedAccessViews(3, 1, &m_SDFBlocksLabelUAV[1], NULL);
		context->CSSetUnorderedAccessViews(5, 1, &m_SDFBlocksLabelUAV[2], NULL);
		context->CSSetUnorderedAccessViews(6, 1, &m_SDFBlocksLabelFrequencyUAV, NULL);
...
/*
HRESULT DX11SceneRepHashSDF::DumpPointCloud( const std::string &filename, ID3D11Device* pDevice, ID3D11DeviceContext* pd3dImmediateContext, unsigned int minWeight, bool justOccupied )
{
	HRESULT hr = S_OK;
...
void DX11SceneRepHashSDF::Alloc(ID3D11DeviceContext* context, ID3D11ShaderResourceView* inputDepth, ID3D11ShaderResourceView* inputColor, ID3D11ShaderResourceView** inputLabel, ID3D11ShaderResourceView* bitMask)
{
	...
	for (int labelno = 0; labelno < LABEL_TEXTURE_NUM; labelno++)
	{
		context->CSSetShaderResources(13+labelno, 1, &inputLabel[labelno]);
	}
	...
	for (int labelno = 0; labelno < LABEL_TEXTURE_NUM; labelno++)
	{
		context->CSSetShaderResources(13+labelno, 1, nullSRV);
	}
...
void DX11SceneRepHashSDF::CompactifyHashEntries( ID3D11DeviceContext* context )
{
	...
	if (false)
	{
		...
		context->CopyResource(m_HashIntegrateDecisionCPU, m_HashIntegrateDecision);
		hr = context->Map(m_HashIntegrateDecisionCPU, 0, D3D11_MAP_READ, 0, &mappedResource);
		if (hr == S_FALSE)
			MessageBox(NULL, L"Save Hash Integrate Decision Failed", L"Failed", MB_OK);

		int* data = (int*)mappedResource.pData; int dataSum = 0;
		for (int i = 0; i < m_HashBucketSize*m_HashNumBuckets; i++)
		{
			dataSum += data[i];
		}

		context->Unmap(m_HashIntegrateDecisionCPU, 0);
	}
	// test

	m_NumOccupiedHashEntries = s_PrefixSumScan.ScanCS(context, m_HashNumBuckets * m_HashBucketSize, 
...
void DX11SceneRepHashSDF::IntegrateDepthMap( ID3D11DeviceContext* context, ID3D11ShaderResourceView* inputDepth, ID3D11ShaderResourceView* inputColor, ID3D11ShaderResourceView** inputLabel)
{
	context->CSSetShaderResources(0, 1, &inputDepth);
	context->CSSetShaderResources(1, 1, &inputColor);
	...
	context->CSSetUnorderedAccessViews(7, 1, nullUAV, NULL);
	context->CSSetUnorderedAccessViews(2, 1, nullUAV, NULL);
	...
	if (false)
	{
		// Copy to CPU
		HRESULT hr = S_OK;
		D3D11_MAPPED_SUBRESOURCE mappedResourceColor, mappedResourceLabel, mappedResourceLabelFrequency;

		context->CopyResource(m_TestIntegrateColorCPU, m_SDFBlocksRGBW);
		context->CopyResource(m_TestIntegrateLabelCPU, m_SDFBlocksLabel[0]);
		context->CopyResource(m_TestIntegrateLabelFrequencyCPU, m_SDFBlocksLabelFrequency);
		V(context->Map(m_TestIntegrateColorCPU, 0, D3D11_MAP_READ, 0, &mappedResourceColor));
		V(context->Map(m_TestIntegrateLabelCPU, 0, D3D11_MAP_READ, 0, &mappedResourceLabel));
		V(context->Map(m_TestIntegrateLabelFrequencyCPU, 0, D3D11_MAP_READ, 0, &mappedResourceLabelFrequency));

		int* dataColor = (int*)mappedResourceColor.pData;
		int* dataLabel = (int*)mappedResourceLabel.pData;
		int* dataLabelFrequency = (int*)mappedResourceLabelFrequency.pData;

		std::string temppathColor = GlobalAppState::getInstance().s_ImageReaderSensorSourcePath + std::string("ColorIntegrate.txt");
		std::string temppathLabel = GlobalAppState::getInstance().s_ImageReaderSensorSourcePath + std::string("LabelIntegrate.txt");
		std::string temppathLabelFrequency = GlobalAppState::getInstance().s_ImageReaderSensorSourcePath + std::string("LabelFrequencyIntegrate.txt");
		std::ofstream tempoutColor(temppathColor.c_str());
		std::ofstream tempoutLabel(temppathLabel.c_str());
		std::ofstream tempoutLabelFrequency(temppathLabelFrequency.c_str());

		int last1, weight, x, y, z;
		int last2, label[4];
		int last3, labelFrequency[12];
		for (int i = 0; i < sizeof(int) * m_SDFBlockSize * m_SDFBlockSize * m_SDFBlockSize * m_SDFNumBlocks; i++)
		{
			last1 = dataColor[i];
			weight = last1 & 0x000000ff;
			last1 >>= 0x8;
			x = last1 & 0x000000ff;
			last1 >>= 0x8;
			y = last1 & 0x000000ff;
			last1 >>= 0x8;
			z = last1 & 0x000000ff;
			tempoutColor << weight << "," << x << "," << y << "," << z << std::endl;

			last2 = dataLabel[i];
			label[0] = last2 & 0x000000ff;
			last2 >>= 0x8;
			label[1] = last2 & 0x000000ff;
			last2 >>= 0x8;
			label[2] = last2 & 0x000000ff;
			last2 >>= 0x8;
			label[3] = last2 & 0x000000ff;
			tempoutLabel << label[0] << "," << label[1] << "," << label[2] << "," << label[3] << std::endl;

			for (int j = 0; j < 6; j++)
			{
				last3 = dataLabelFrequency[6 * i + j];
				labelFrequency[2 * j + 0] = last3 & 0x0000ffff;
				last3 >>= 0x10;
				labelFrequency[2 * j + 1] = last3 & 0x0000ffff;
				tempoutLabelFrequency << labelFrequency[2 * j + 0] << ", " << labelFrequency[2 * j + 1] << ", ";
			}
			tempoutLabelFrequency << std::endl;
		}

		tempoutColor.close();
		tempoutLabel.close();
		tempoutLabelFrequency.close();
		context->Unmap(m_TestIntegrateColorCPU, 0);
		context->Unmap(m_TestIntegrateLabelCPU, 0);
		context->Unmap(m_TestIntegrateLabelFrequencyCPU, 0);
	}
}
...
void DX11SceneRepHashSDF::GarbageCollect( ID3D11DeviceContext* context )
{
	...
	{
		context->CSSetShaderResources(9+i, 1, &m_SDFBlocksLabelSRV[i]);
	}
	context->CSSetShaderResources(12, 1, &m_SDFBlocksLabelFrequencySRV);
	...
	context->CSSetShaderResources(12, 1, nullSRV);
	...
	context->CSSetUnorderedAccessViews(2, 1, nullUAV, NULL);
	context->CSSetUnorderedAccessViews(3, 1, nullUAV, NULL);
	context->CSSetUnorderedAccessViews(4, 1, nullUAV, NULL);
	context->CSSetUnorderedAccessViews(5, 1, nullUAV, NULL);
	context->CSSetUnorderedAccessViews(6, 1, nullUAV, NULL);
	context->CSSetUnorderedAccessViews(7, 1, nullUAV, NULL);
...
HRESULT DX11SceneRepHashSDF::CreateBuffers( ID3D11Device* pd3dDevice )
{
	...
	if (!m_JustHashAndNoSDFBlocks)
	{
		...
		//create sdf blocks (8x8x8 -> 8 byte per voxel: Label part)
		ZeroMemory(&descBUF, sizeof(D3D11_BUFFER_DESC));
		descBUF.BindFlags = D3D11_BIND_UNORDERED_ACCESS | D3D11_BIND_SHADER_RESOURCE;
		descBUF.Usage = D3D11_USAGE_DEFAULT;
		descBUF.CPUAccessFlags = 0;
		...
		for (int i = 0; i < LABEL_TEXTURE_NUM; i++)
		{
			...
			ZeroMemory(cpuNull, sizeof(int)*m_SDFBlockSize*m_SDFBlockSize*m_SDFBlockSize*m_SDFNumBlocks);
			InitData.pSysMem = cpuNull;
			V_RETURN(pd3dDevice->CreateBuffer(&descBUF, &InitData, &m_SDFBlocksLabel[i]));
			SAFE_DELETE_ARRAY(cpuNull);
	
			V_RETURN(pd3dDevice->CreateShaderResourceView(m_SDFBlocksLabel[i], &descSRV, &m_SDFBlocksLabelSRV[i]));
			V_RETURN(pd3dDevice->CreateUnorderedAccessView(m_SDFBlocksLabel[i], &descUAV, &m_SDFBlocksLabelUAV[i]));
		}

		//create sdf blocks (8x8x8 -> 8 byte per voxel: Label Frequency)
		ZeroMemory(&descBUF, sizeof(D3D11_BUFFER_DESC));
		descBUF.BindFlags = D3D11_BIND_UNORDERED_ACCESS | D3D11_BIND_SHADER_RESOURCE;
		descBUF.Usage = D3D11_USAGE_DEFAULT;
		descBUF.CPUAccessFlags = 0;
		descBUF.MiscFlags = 0;
		descBUF.ByteWidth = 6 * sizeof(int) * m_SDFBlockSize * m_SDFBlockSize * m_SDFBlockSize * m_SDFNumBlocks;
		descBUF.StructureByteStride = sizeof(int);
		...
		descSRV.ViewDimension = D3D11_SRV_DIMENSION_BUFFER;
		descSRV.Buffer.FirstElement = 0;
		descSRV.Buffer.NumElements = 6 * m_SDFBlockSize * m_SDFBlockSize * m_SDFBlockSize * m_SDFNumBlocks;

		ZeroMemory(&descUAV, sizeof(D3D11_UNORDERED_ACCESS_VIEW_DESC));
		descUAV.Format = DXGI_FORMAT_R32_SINT;;
		descUAV.ViewDimension = D3D11_UAV_DIMENSION_BUFFER;
		descUAV.Buffer.FirstElement = 0;
		descUAV.Buffer.NumElements = 6 * m_SDFBlockSize * m_SDFBlockSize * m_SDFBlockSize * m_SDFNumBlocks;

		ZeroMemory(&InitData, sizeof(D3D11_SUBRESOURCE_DATA));
		cpuNull = new int[6*m_SDFBlockSize*m_SDFBlockSize*m_SDFBlockSize*m_SDFNumBlocks];
		ZeroMemory(cpuNull, 6*sizeof(int)*m_SDFBlockSize*m_SDFBlockSize*m_SDFBlockSize*m_SDFNumBlocks);
		InitData.pSysMem = cpuNull;
		V_RETURN(pd3dDevice->CreateBuffer(&descBUF, &InitData, &m_SDFBlocksLabelFrequency));
		SAFE_DELETE_ARRAY(cpuNull);

		V_RETURN(pd3dDevice->CreateShaderResourceView(m_SDFBlocksLabelFrequency, &descSRV, &m_SDFBlocksLabelFrequencySRV));
		V_RETURN(pd3dDevice->CreateUnorderedAccessView(m_SDFBlocksLabelFrequency, &descUAV, &m_SDFBlocksLabelFrequencyUAV));
		...
		V_RETURN(pd3dDevice->CreateBuffer(&descBUF, NULL, &m_TestIntegrateColorCPU));
		V_RETURN(pd3dDevice->CreateBuffer(&descBUF, NULL, &m_TestIntegrateLabelCPU));

		descBUF.ByteWidth = 6 * sizeof(int) * m_SDFBlockSize * m_SDFBlockSize * m_SDFBlockSize * m_SDFNumBlocks;
		V_RETURN(pd3dDevice->CreateBuffer(&descBUF, NULL, &m_TestIntegrateLabelFrequencyCPU));
		// test
	}
	...
	InitData.pSysMem = cpuNull;
	V_RETURN(pd3dDevice->CreateBuffer(&descBUF, &InitData, &m_HashIntegrateDecisionCPU));
	SAFE_DELETE_ARRAY(cpuNull);
	// test
