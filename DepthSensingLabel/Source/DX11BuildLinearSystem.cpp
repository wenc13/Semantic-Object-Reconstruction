...
HRESULT DX11BuildLinearSystem::applyBL(ID3D11DeviceContext* context, 
	                                   ID3D11ShaderResourceView* inputSRV, ID3D11ShaderResourceView* inputNormalsSRV, ID3D11ShaderResourceView* correspondenceSRV, ID3D11ShaderResourceView* correspondenceNormalsSRV, 
	                                   D3DXVECTOR3& mean, float meanStDev, Eigen::Matrix4f& deltaTransform, unsigned int imageWidth, unsigned int imageHeight, unsigned int level, Matrix6x7f& res, LinearSystemConfidence& conf, float focalx, float focaly)
{
	HRESULT hr = S_OK;
	...
	cbuffer->focalx          = focalx;
	cbuffer->focaly          = focalx;
	...
	context->CSSetShaderResources(1, 1, &inputNormalsSRV);
	context->CSSetShaderResources(2, 1, &correspondenceSRV);
	context->CSSetShaderResources(3, 1, &correspondenceNormalsSRV);
	...
	context->CSSetShaderResources(3, 1, nullSAV);
...
Matrix6x7f DX11BuildLinearSystem::reductionSystemCPU( const float* data, unsigned int nElems, LinearSystemConfidence& conf )
{
	...
	res /= (float)numCorrF;
	conf.sumRegError /= (float)numCorrF;
	conf.sumRegWeight /= (float)numCorrF;
