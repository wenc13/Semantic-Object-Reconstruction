...
#define LABEL_TEXTURE_NUM 3
#define LABEL_TEXTURE_CHANNEL 4
...
class DepthSensor
{
	public:
		...
		virtual HRESULT processLabel() = 0;

		//! Processes the previous depth data 
		virtual HRESULT processPreDepth() = 0;

		//! Processes the previous color data 
		virtual HRESULT processPreColor() = 0;

		//! Processes the previous label data
		virtual HRESULT processPreLabel() = 0;
		...
		// test
		virtual void setCurrentFrameNumber(unsigned int i) = 0;
		// test
		...
		//! gets the pointer to label array
		BYTE **getLabel() { return m_label; }

		//! gets the pointer to previous depth array
		USHORT *getPreDepthD16() { return m_preDepthD16; }

		//! gets the pointer to previous color array
		BYTE *getPreColorRGBX() { return m_preColorRGBX; }

		//! gets the pointer to previous label array
		BYTE **getPreLabel() { return m_preLabel; }
	...
	protected:
		//! depth ,color and label data filled by respective child classes
		USHORT* m_depthD16;
		BYTE* m_colorRGBX;
		BYTE* m_label[LABEL_TEXTURE_NUM];		//divide the label data to several parts, each part has the same format as RGB 

		USHORT* m_preDepthD16;
		BYTE* m_preColorRGBX;
		BYTE* m_preLabel[LABEL_TEXTURE_NUM];
	...
	private:
		...
		void computePointCurrentPointCloud(PointCloudf& pc, const mat4f& transform = mat4f::identity()) const 
		{
			...
			if (!(getColorWidth() == getDepthWidth() && getColorHeight() == getDepthHeight()))	
				throw MLIB_EXCEPTION("invalid dimensions");
