...
HRESULT DX11SceneRepChunkGrid::StreamOutToCPUPass0GPU(ID3D11DeviceContext* context, DX11SceneRepHashSDF& sceneRepHashSDF, const vec3f& posCamera, float radius, bool useParts, bool multiThreaded)
{
	...
	ID3D11UnorderedAccessView** sdfBlocksLabelUAV          = sceneRepHashSDF.GetSDFBlocksLabelUAV();
	ID3D11UnorderedAccessView*  sdfBlocksLabelFrequencyUAV = sceneRepHashSDF.GetSDFBlocksLabelFrequencyUAV();
	...
	context->CSSetUnorderedAccessViews(3, 1, &hashBucketMutex, 0);
	...
	V_RETURN(context->Map(m_BuffCountSDFBlockOutput, 0, D3D11_MAP_READ, 0, &mappedResource));
	...
	if (nSDFBlockDescs != 0)
	{
		...
		{
			context->CSSetUnorderedAccessViews(3+labelno, 1, &sdfBlocksLabelUAV[labelno], 0);
		}
		context->CSSetUnorderedAccessViews(6, 1, &sdfBlocksLabelFrequencyUAV, 0);
		...
		context->CSSetUnorderedAccessViews(3, 3, nullUAV2, 0);
		context->CSSetUnorderedAccessViews(6, 1, nullUAV2, 0);
		...
		const unsigned int linBlockSize = 11*SDF_BLOCK_SIZE*SDF_BLOCK_SIZE*SDF_BLOCK_SIZE;
		...
		D3D11_BOX sourceRegionBlock = {0, 0, 0, linBlockSize*sizeof(int)*nSDFBlockDescs, 1, 1};
...
HRESULT DX11SceneRepChunkGrid::StreamInToGPUPass1GPU(ID3D11DeviceContext* context, DX11SceneRepHashSDF& sceneRepHashSDF, bool multiThreaded)
{
	...
	if (s_nStreamdInBlocks != 0)
	{
		...
		D3D11_BOX sourceRegionBlock = {0, 0, 0, linBlockSize*sizeof(int)*s_nStreamdInBlocks, 1, 1};
