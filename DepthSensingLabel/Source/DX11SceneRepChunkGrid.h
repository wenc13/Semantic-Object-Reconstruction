...
class DX11SceneRepChunkGrid
{
	public:
		...
		HRESULT Init(ID3D11Device* pd3dDevice, ID3D11DeviceContext* context, vec3f& voxelExtends, vec3i gridDimensions, vec3i minGridPos, float virtualVoxelSize, unsigned int initialChunkListSize)
		{
			...
			// Create Append Buffer CPU Output
			...
			unsigned int linBlockSize = (2+3+6)*SDF_BLOCK_SIZE*SDF_BLOCK_SIZE*SDF_BLOCK_SIZE;
						...
						if (containsSDFBlocksChunk(chunk))
						{
							...
							if (m_grid[index] != NULL && m_grid[index]->isStreamedOut()) // As been allocated and has streamed out blocks
							{
								...
								for (size_t i = 0; i < chunkDesc->m_ChunkDesc.size(); i++)
								{
									...
									{
										int first = chunkDesc->m_SDFBlocks[i].data[11*j+0];
										vBlock.voxels[j].sdf = *(float*)(&first);

										int last; 
										last = chunkDesc->m_SDFBlocks[i].data[11*j+1];
										...
										int last2;
										last2 = chunkDesc->m_SDFBlocks[i].data[11*j+2];
										vBlock.voxels[j].label[0]  = last2 & 0x000000ff;
										last2 >>= 0x8;
										vBlock.voxels[j].label[1]  = last2 & 0x000000ff;
										last2 >>= 0x8;
										vBlock.voxels[j].label[2]  = last2 & 0x000000ff;
										last2 >>= 0x8;
										vBlock.voxels[j].label[3]  = last2 & 0x000000ff;

										last2 = chunkDesc->m_SDFBlocks[i].data[11*j+3];
										vBlock.voxels[j].label[4]  = last2 & 0x000000ff;
										last2 >>= 0x8;
										vBlock.voxels[j].label[5]  = last2 & 0x000000ff;
										last2 >>= 0x8;
										vBlock.voxels[j].label[6]  = last2 & 0x000000ff;
										last2 >>= 0x8;
										vBlock.voxels[j].label[7]  = last2 & 0x000000ff;

										last2 = chunkDesc->m_SDFBlocks[i].data[11*j+4];
										vBlock.voxels[j].label[8]  = last2 & 0x000000ff;
										last2 >>= 0x8;
										vBlock.voxels[j].label[9]  = last2 & 0x000000ff;
										last2 >>= 0x8;
										vBlock.voxels[j].label[10] = last2 & 0x000000ff;
										last2 >>= 0x8;
										vBlock.voxels[j].label[11] = last2 & 0x000000ff;

										int last3;
										for (unsigned int k = 0; k < 6; k++)
										{
											last3 = chunkDesc->m_SDFBlocks[i].data[11*j+5+k];
											vBlock.voxels[j].labelFrequency[2*k+0] = last3 & 0x0000ffff;
											last3 >>= 0x10;
											vBlock.voxels[j].labelFrequency[2*k+1] = last3 & 0x0000ffff;
										}
									}
