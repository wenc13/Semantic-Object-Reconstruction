...
Buffer<int> g_SDFBlocksLabel_1        : register(t3);
Buffer<int> g_SDFBlocksLabel_2        : register(t4);
Buffer<int> g_SDFBlocksLabel_3        : register(t5);
Buffer<int> g_SDFBlocksLabelFrequency : register(t6);

AppendStructuredBuffer<Triangle>     g_triangles     : register(u0);
AppendStructuredBuffer<Normal>       g_normals       : register(u1);
AppendStructuredBuffer<Label>        g_labels        : register(u2);
AppendStructuredBuffer<Confidence>   g_confidence    : register(u3);
AppendStructuredBuffer<VirtualVoxel> g_virtualvoxels : register(u4);
...
void extractIsoSurfaceHashSDFCS(int3 dTid : SV_DispatchThreadID, uint3 GTid : SV_GroupThreadID, uint GI: SV_GroupIndex, uint3 GID : SV_GroupID)
{
	...
	if (groupID < g_HashNumBuckets*g_HashBucketSize)
	{
		...
		if (entry.ptr != FREE_ENTRY)
		{
			...
			int4 label_1; int4 label_2; int4 label_3;
			
			float3 p000 = worldPos+float3(M, M, M); float dist000; float3 color000; bool valid000 = trilinearInterpolationSimpleFastFast(p000, dist000, color000, label_1, label_2, label_3);
			float3 p100 = worldPos+float3(P, M, M); float dist100; float3 color100; bool valid100 = trilinearInterpolationSimpleFastFast(p100, dist100, color100, label_1, label_2, label_3);
			float3 p010 = worldPos+float3(M, P, M); float dist010; float3 color010; bool valid010 = trilinearInterpolationSimpleFastFast(p010, dist010, color010, label_1, label_2, label_3);
			float3 p001 = worldPos+float3(M, M, P); float dist001; float3 color001; bool valid001 = trilinearInterpolationSimpleFastFast(p001, dist001, color001, label_1, label_2, label_3);
			float3 p110 = worldPos+float3(P, P, M); float dist110; float3 color110; bool valid110 = trilinearInterpolationSimpleFastFast(p110, dist110, color110, label_1, label_2, label_3);
			float3 p011 = worldPos+float3(M, P, P); float dist011; float3 color011; bool valid011 = trilinearInterpolationSimpleFastFast(p011, dist011, color011, label_1, label_2, label_3);
			float3 p101 = worldPos+float3(P, M, P); float dist101; float3 color101; bool valid101 = trilinearInterpolationSimpleFastFast(p101, dist101, color101, label_1, label_2, label_3);
			float3 p111 = worldPos+float3(P, P, P); float dist111; float3 color111; bool valid111 = trilinearInterpolationSimpleFastFast(p111, dist111, color111, label_1, label_2, label_3);
            ...
            Voxel v = getVoxel(g_SDFBlocksSDF, g_SDFBlocksRGBW, g_SDFBlocksLabel_1, g_SDFBlocksLabel_2, g_SDFBlocksLabel_3, g_SDFBlocksLabelFrequency, entry.ptr+linearizeVoxelPos(GTid));
            ...
            uint maxIndex = 0; float maxLabel = 0.0f; float entropy = 0.0f;
            for (uint l = 0; l < labelNum; l++)
            {
                if (v.label[l]/255.0f > maxLabel)
                {
                    maxIndex = l;
                    maxLabel = v.label[l]/255.0f;
                }

                float pp = v.label[l]/255.0f;
                if (pp != 0.0f)
                    entropy -= pp * log(pp);
            }
            float confidence = 1-entropy/log(labelNum);

            if (maxIndex == 0 && confidence > 0.7f)
            {
                isObject = false;
            }
            ...
            if (true)
            {
                ...
                for (int i = 0; triTable[cubeindex][i] != -1; i += 3)
                {
                    ...
                    if (maxIndex == 1)
                    {
                        t.v0.c = float3(255.0f, 255.0f, 0.0f) / 255.0f;
                        t.v1.c = float3(255.0f, 255.0f, 0.0f) / 255.0f;
                        t.v2.c = float3(255.0f, 255.0f, 0.0f) / 255.0f;
                    }
                    else if (maxIndex == 2)
                    {
                        t.v0.c = float3(0.0f, 89.0f, 0.0f) / 255.0f;
                        t.v1.c = float3(0.0f, 89.0f, 0.0f) / 255.0f;
                        t.v2.c = float3(0.0f, 89.0f, 0.0f) / 255.0f;
                    }
                    else if (maxIndex == 3)
                    {
                        t.v0.c = float3(0.0f, 255.0f, 255.0f) / 255.0f;
                        t.v1.c = float3(0.0f, 255.0f, 255.0f) / 255.0f;
                        t.v2.c = float3(0.0f, 255.0f, 255.0f) / 255.0f;
                    }
                    else if (maxIndex == 4)
                    {
                        t.v0.c = float3(85.0f, 170.0f, 255.0f) / 255.0f;
                        t.v1.c = float3(85.0f, 170.0f, 255.0f) / 255.0f;
                        t.v2.c = float3(85.0f, 170.0f, 255.0f) / 255.0f;
                    }
                    else
                    {
                        t.v0.c = float3(255.0f, 255.0f, 255.0f) / 255.0f;
                        t.v1.c = float3(255.0f, 255.0f, 255.0f) / 255.0f;
                        t.v2.c = float3(255.0f, 255.0f, 255.0f) / 255.0f;
                    }
            ...
            */
            ...
            {
                // Table
                triColor[0] = float3(225.0f, 225.0f, 225.0f); triColor[1] = float3(255.0f, 219.0f, 100.0f); triColor[2]  = float3(157.0f, 197.0f, 233.0f); triColor[3]  = float3(255.0f, 255.0f, 255.0f);
			    triColor[4] = float3(255.0f, 255.0f, 255.0f); triColor[5] = float3(255.0f, 255.0f, 255.0f); triColor[6]  = float3(255.0f, 255.0f, 255.0f); triColor[7]  = float3(255.0f, 255.0f, 255.0f);
			    triColor[8] = float3(255.0f, 255.0f, 255.0f); triColor[9] = float3(255.0f, 255.0f, 255.0f); triColor[10] = float3(255.0f, 255.0f, 255.0f); triColor[11] = float3(255.0f, 255.0f, 255.0f);
            }
            else if (labelNum == 5)
            {
                // Chair
				triColor[0] = float3(225.0f, 225.0f, 225.0f); triColor[1] = float3(243.0f, 177.0f, 133.0f); triColor[2]  = float3(255.0f, 219.0f, 100.0f); triColor[3]  = float3(157.0f, 233.0f, 225.0f);
				triColor[4] = float3(157.0f, 197.0f, 233.0f); triColor[5] = float3(255.0f, 255.0f, 255.0f); triColor[6]  = float3(255.0f, 255.0f, 255.0f); triColor[7]  = float3(255.0f, 255.0f, 255.0f);
				triColor[8] = float3(255.0f, 255.0f, 255.0f); triColor[9] = float3(255.0f, 255.0f, 255.0f); triColor[10] = float3(255.0f, 255.0f, 255.0f); triColor[11] = float3(255.0f, 255.0f, 255.0f);
            }
            else if (labelNum == 7)
            {
                // Motorcycle
				triColor[0] = float3(225.0f, 225.0f, 225.0f); triColor[1] = float3(157.0f, 197.0f, 233.0f); triColor[2]  = float3(157.0f, 233.0f, 225.0f); triColor[3]  = float3(233.0f, 157.0f, 165.0f);
		    	triColor[4] = float3(167.0f, 211.0f, 143.0f); triColor[5] = float3(255.0f, 219.0f, 100.0f); triColor[6]  = float3(243.0f, 177.0f, 133.0f); triColor[7]  = float3(255.0f, 255.0f, 255.0f);
		    	triColor[8] = float3(255.0f, 255.0f, 255.0f); triColor[9] = float3(255.0f, 255.0f, 255.0f); triColor[10] = float3(255.0f, 255.0f, 255.0f); triColor[11] = float3(255.0f, 255.0f, 255.0f);
            }
            ...
            for (int i = 0; triTable[cubeindex][i] != -1; i += 3)
            {
                ...
                // labeled color
                t.v0.c = triColor[maxIndex]/255.0f;
                t.v1.c = triColor[maxIndex]/255.0f;
                t.v2.c = triColor[maxIndex]/255.0f;

                g_triangles.Append(t);

                // Normals
                float3 n1 = t.v1.p-t.v0.p;
                float3 n2 = t.v2.p-t.v1.p;
                float3 n3 = float3(n1.y*n2.z-n1.z*n2.y, n1.z*n2.x-n1.x*n2.z, n1.x*n2.y-n1.y*n2.x);
                
                Normal norm;
                norm.n = normalize(n3);
                g_normals.Append(norm);

                // Labels
                Label lab;

                int last = 0;
                last |= v.label[3]  & 0x000000ff;
                last <<= 8;
                last |= v.label[2]  & 0x000000ff;
                last <<= 8;
                last |= v.label[1]  & 0x000000ff;
                last <<= 8;
                last |= v.label[0]  & 0x000000ff;
                lab.label.x = last;

                last = 0;
                last |= v.label[7]  & 0x000000ff;
                last <<= 8;
                last |= v.label[6]  & 0x000000ff;
                last <<= 8;
                last |= v.label[5]  & 0x000000ff;
                last <<= 8;
                last |= v.label[4]  & 0x000000ff;
                lab.label.y = last;

                last = 0;
                last |= v.label[11] & 0x000000ff;
                last <<= 8;
                last |= v.label[10] & 0x000000ff;
                last <<= 8;
                last |= v.label[9]  & 0x000000ff;
                last <<= 8;
                last |= v.label[8]  & 0x000000ff;
                lab.label.z = last;
                ...
                {
                    float pp = (float)v.label[k]/255.0f;
                ...
                Confidence confid;
                confid.confidence = 1-entropy/log(labelNum);
                g_confidence.Append(confid);

                // Virual Voxels
                VirtualVoxel virtualvoxel;
                virtualvoxel.vVoxelPos = pi;
                g_virtualvoxels.Append(virtualvoxel);
            }
		}
	}
}