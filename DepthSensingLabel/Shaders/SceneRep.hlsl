...
void setVoxel(RWBuffer<int> hash, in uint id, in Voxel voxel) 
{
	...
	//if (hash[4*id+0] != voxel.pos.x || hash[4*id+1] != voxel.pos.y || hash[4*id+2] != voxel.pos.z)
    //    hash[4*id+3] = 0;
}
...
{
	return int3((int)((pos.x * g_VirtualVoxelResolutionScalar) + 0.5f),
...
void integrateVoxel(RWBuffer<int> hash, in Voxel voxel)
{
	...
	for (uint j = 0; j < g_HashBucketSize; j++) 
    {
        ...
        {
			firstEmpty = i;
		} else if (curr.pos.x == voxel.pos.x && curr.pos.y == voxel.pos.y && curr.pos.z == voxel.pos.z && curr.weight > 0 && curr.weight != wLock)
        {	
            //this is hacky: hash[4*i+3] must be read from register
...
void integrateVoxelToOther(RWBuffer<int> hash, in Voxel voxel)
{
	...
	uint firstEmpty = (uint)-1;
	[unroll]
	for (uint j = 0; j < g_HashBucketSize && j < 10; j++) 
    {
		uint i = j + hp;
		Voxel curr = getVoxel(hash, i);
