...
RWBuffer<int>   g_SDFBlocksLabel_1        : register(u3);
RWBuffer<int>   g_SDFBlocksLabel_2        : register(u4);
RWBuffer<int>   g_SDFBlocksLabel_3        : register(u5);
RWBuffer<int>   g_SDFBlocksLabelFrequency : register(u6);
...
void integrateFromGlobalHashPass2CS(int3 dTid : SV_DispatchThreadID, uint3 GTid : SV_GroupThreadID, uint GI: SV_GroupIndex, uint3 GID : SV_GroupID)
{
	...
	if (idxBlock < nSDFBlockDescs)
	{
		...
		g_outputSDFBlocks[11*(linBlockSize*idxBlock+GI)+0] = asint(g_SDFBlocksSDF[desc.ptr+GI]);
		g_outputSDFBlocks[11*(linBlockSize*idxBlock+GI)+1] = g_SDFBlocksRGBW[desc.ptr+GI];
        g_outputSDFBlocks[11*(linBlockSize*idxBlock+GI)+2] = g_SDFBlocksLabel_1[desc.ptr+GI];
        g_outputSDFBlocks[11*(linBlockSize*idxBlock+GI)+3] = g_SDFBlocksLabel_2[desc.ptr+GI];
        g_outputSDFBlocks[11*(linBlockSize*idxBlock+GI)+4] = g_SDFBlocksLabel_3[desc.ptr+GI];
        for (uint i = 0; i < 6; i++)
        {
            g_outputSDFBlocks[11*(linBlockSize*idxBlock+GI)+5+i] = g_SDFBlocksLabelFrequency[6*(desc.ptr+GI)+i];
        }

		// Reset SDF Block
        deleteVoxel(g_SDFBlocksSDF, g_SDFBlocksRGBW, g_SDFBlocksLabel_1, g_SDFBlocksLabel_2, g_SDFBlocksLabel_3, g_SDFBlocksLabelFrequency, desc.ptr+GI);
    }
}