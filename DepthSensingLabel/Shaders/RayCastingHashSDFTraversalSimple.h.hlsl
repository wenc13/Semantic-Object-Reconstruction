...
/*float3 gradientForPoint(float3 pos)
{
	...
	float distp00; float3 colorp00; trilinearInterpolationSimpleFastFast(pos-float3(0.5f*offset.x, 0.0f, 0.0f), distp00, colorp00,label_1, label_2,label_3);
	float dist0p0; float3 color0p0; trilinearInterpolationSimpleFastFast(pos-float3(0.0f, 0.5f*offset.y, 0.0f), dist0p0, color0p0,label_1, label_2,label_3);
	float dist00p; float3 color00p; trilinearInterpolationSimpleFastFast(pos-float3(0.0f, 0.0f, 0.5f*offset.z), dist00p, color00p,label_1, label_2,label_3);

	float dist100; float3 color100; trilinearInterpolationSimpleFastFast(pos+float3(0.5f*offset.x, 0.0f, 0.0f), dist100, color100,label_1, label_2,label_3);
	float dist010; float3 color010; trilinearInterpolationSimpleFastFast(pos+float3(0.0f, 0.5f*offset.y, 0.0f), dist010, color010,label_1, label_2,label_3);
	float dist001; float3 color001; trilinearInterpolationSimpleFastFast(pos+float3(0.0f, 0.0f, 0.5f*offset.z), dist001, color001,label_1, label_2,label_3);
...
/*void traverseCoarseGridSimpleSampleAll(float3 worldCamPos, float3 worldDir, float3 camDir, int3 dTid)
{
	...
	while (rayCurrent < rayEnd)
	{
		float3 currentPosWorld = worldCamPos+rayCurrent*worldDir;
		HashEntry entry = getHashEntryForSDFBlockPos(g_Hash, worldToSDFBlock(currentPosWorld));
		float dist;	float3 color; int4 label_1; int4 label_2; int4 label_3;
			...
			{					
				...
				if (abs(lastSample.sdf - dist) < g_thresSampleDist)
				{
					if (abs(dist) < g_thresDist)
					{
						g_output[dTid.xy] = alpha/depthToRayLength; // Convert ray length to depth depthToRayLength
						g_outputColor[dTid.xy]   = float4(color/255.0f, 1.0f);
						g_outputLabel_1[dTid.xy] = float4(label_1/100.0f); 
						g_outputLabel_2[dTid.xy] = float4(label_2/100.0f); 
						g_outputLabel_3[dTid.xy] = float4(label_3/100.0f); 
