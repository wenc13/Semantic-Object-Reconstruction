...
Texture2D<float4> inputNormals               : register(t1);
Texture2D<float4> target                     : register(t2);
Texture2D<float4> targetNormals              : register(t3);
...
Float1x6 buildRowSystemMatrixPlaneColorAndLabel(float3 o, float fx, float fy, float gx, float gy, float w)
{
	Float1x6 row;
	row.data[2] = gx*fx*(-o.y)/(o.z)+gy*fy*(o.x)/(o.z);
	row.data[1] = gx*fx*(o.x*o.x+o.z*o.z)/(o.z*o.z)+gy*fy*(o.x*o.y)/(o.z*o.z);
	row.data[0] = gx*fx*(-o.x*o.y)/(o.z*o.z)+gy*fy*(-o.y*o.y-o.z*o.z)/(o.z*o.z);

	row.data[3] = gx*fx*(1.0f)/(o.z);
	row.data[4] = gy*fy*(1.0f)/(o.z);
	row.data[5] = gx*fx*(-o.x)/(o.z*o.z)+gy*fy*(-o.y)/(o.z*o.z);
...
float buildRowRHSPlaneColorAndLabel(float p, float q, float w)
{
	return (q-p);
}
...
void addToLocalSystem(float3 p, float3 q, float3 n, float weight, uint GTid)
{
    ...
    bucket[GTid].data[27] += weight*dN*dN;    // residual has to be rescaled on the CPU if used, because of the variance normalization !!!
